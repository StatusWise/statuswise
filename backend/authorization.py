...
from models import User, Project, Incident


class AuthorizationService:
    def __init__(self, model_path: str = "rbac_model.conf", policy_path: str = "rbac_policy.csv"):
        self.enforcer = casbin.Enforcer(model_path, policy_path)
    
    def check_project_access(self, user_id: int, project_id: int, action: str, db: Session) -> bool:
    ...
    def check_incident_access(self, user_id: int, incident_id: int, action: str, db: Session) -> bool:
    ...
    def check_project_for_incident_creation(self, user_id: int, project_id: int, db: Session) -> bool:
...
def require_project_access(user: User, project_id: int, action: str, db: Session, auth_service: Optional[AuthorizationService] = None):
...
def require_incident_access(user: User, incident_id: int, action: str, db: Session, auth_service: Optional[AuthorizationService] = None):
